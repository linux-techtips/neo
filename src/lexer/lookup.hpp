...
constexpr auto pack(str text) -> u64 {
  ...
  if (__builtin_is_constant_evaluated()) {
    for (auto i = 0u; i < text.size(); ++i) data[i] = static_cast<u8>(text[i]);
...
constexpr auto keyword(str text) -> Token::Kind {
  ...
  if (slot >= Keyword::Size or Keyword::Table.texts[slot] == data) return Token::Ident;

  return Keyword::Table.kinds[slot];
}
